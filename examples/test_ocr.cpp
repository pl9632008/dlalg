...
int main(int argc, char ** argv)
{
    ...
    if(argc != 3)
    {
        std::cout<<"input error,please input: ./yjh_deeplearning_test_ocr <ocr_alg_cfg_path> <image_address>"<<std::endl;
        exit(0);
    }
   ...
   for(unsigned int i=0;i<output_info.result_list.size();i++)
   {
        std::string tms;
        int start_index = 0;
        tms = output_info.result_list[i].value.substr(start_index,output_info.result_list[i].value.size()/2);
        std::cout<<tms<<std::endl;
        if(output_info.result_list[i].value.size() % 2 == 0)
        {
            start_index = output_info.result_list[i].value.size()/2;
        }
        else
        {
            start_index = output_info.result_list[i].value.size()/2+1;
        }
        tms = output_info.result_list[i].value.substr(start_index,output_info.result_list[i].value.size()/2);
        std::cout<<tms<<std::endl;
        std::cout<<output_info.result_list[i].value.size()<<" "<<output_info.result_list[i].value<<" "<<output_info.result_list[i].score<<" "<<output_info.result_list[i].center_x<<" "<<output_info.result_list[i].center_y<<" "<<output_info.result_list[i].width<<" "<<output_info.result_list[i].height<<" "<<output_info.result_list[i].angle<<std::endl;
