...
int main(int argc, char ** argv)
{
    ...
    if(argc != 3)
    {
        std::cout<<"input error,please input: ./yjh_deeplearning_test_orientedrcnn <orientedrcnn_alg_cfg_path> <image_address>"<<std::endl;
        exit(0);
    }
    ...
    yjh_deeplearning::AIAlgorithm oriened_rcnn;


    int ret;
    ret = oriened_rcnn.Init(argv[1],0);
    if(ret != 0)
    {
    ...
    auto beforeTime = std::chrono::steady_clock::now();
        for(int i=0;i<10;i++)
    ret = oriened_rcnn.Inference(input_info,output_info);
   ...
   for(unsigned int i=0;i<output_info.result_list.size();i++)
   {
        ...
        rectInput.angle = output_info.result_list[i].angle/PI*180;
     
        if(rectInput.angle < 0)
        {
            rectInput.angle += 180;
        }
        if(output_info.result_list[i].value == "wcgz")
        {
             DrawRotatedRect(test_image, rectInput, cv::Scalar(255, 0, 0));
        }
        else
        {
