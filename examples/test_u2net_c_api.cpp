...
int main(int argc, char ** argv)
{
    ...
    if(argc != 3)
    {
        std::cout<<"input error,please input: ./yjh_deeplearning_test_u2net_base <u2net_alg_cfg_path> <image_address>"<<std::endl;
        exit(0);
    }
   ...
   std::shared_ptr<void> unet = yjh_deeplearning::GetAlgorithmInstance();


    int ret;
    ret = yjh_deeplearning::InitAlgorithm(unet,argv[1],0);
    if(ret != 0)
    {
    ...
    yjh_deeplearning::AlgInputInfo input_info;
    yjh_deeplearning::AlgOutputInfo output_info;
    ...
    ret = yjh_deeplearning::AlgorithmInference(unet,input_info,output_info);
